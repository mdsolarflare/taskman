...
- **Sample YAML hierarchy quirk** — `frontend/public/sample.yaml` node 14 (`Implement Pan & Zoom`) lists `subtask_ids: [12]` (Integration Tests), which creates an unexpected cross-branch parent-child link. Verify this is intentional or fix. #note its intentional.
...
instead setup project coming from pnpm to install nodejs (or later deno?). this would allow us to skip over npm in it's entirety.
