...
- **First visit (online):** the service worker installs and precaches the
  whole shell; the page is then controlled by it.
- **Offline:** every load is served from the precache — including the WASM
  brain and the sample graph — so the app fully works with zero network.
  Any workspace you've edited lives in `localStorage` (and, if tracked, your
  auto-save file), so your data is intact offline.
- **Not precached:** nothing — the shell is fully offline-capable. (Data is
  local-first; there is no backend to reach.)

### Update semantics

The build emits `dist/cache-manifest.js` — a SHA-256 revision map over every
shell file (generated by `deno task gen-revision`, part of `deno task
build`). The service worker names its cache after the manifest's `buildId`:

1. A new deploy changes some file → new `buildId` → new cache name.
2. The browser fetches the new `sw.js` (always revalidated —
   `updateViaCache: "none"`), which imports the new manifest and installs.
3. The new worker precaches the fresh shell, then **waits**. A
   "A new version is ready — Reload" toast appears.
4. Clicking **Reload** sends `SKIP_WAITING`; the new worker activates,
   deletes every other build's caches, and the page reloads once onto the
   new shell.

Because the bundle filenames are stable (`dist/main.js`, not
`main.<hash>.js`), this revision map is the only staleness guard — without
it the cache would serve the old shell forever.

Cache delivery in one line: **cache-first for the shell, network only for
the first uncached GET** — the SW serves from `caches`, and the precache is
rebuilt (not patched) on every deploy.
...
# from repo root
deno task gen-icons
# (runs frontend/e2e/gen_icons.ts in headless Edge — no new dependencies)
```
